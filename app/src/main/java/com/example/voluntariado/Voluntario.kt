...
data class Voluntario(var id: Long = -1, var nome: String, var idade: Long = 0, var telefone: Long = 0, var genero: String) {
    fun toContentValues(): ContentValues {
        val valores = ContentValues()
        valores.put(TabelaVoluntarios.CAMPO_NOME, nome)
        valores.put(TabelaVoluntarios.CAMPO_IDADE, idade)
    ...
    companion object {
        fun fromCursor(cursor: Cursor): Voluntario {
            ...
            val volIdade = cursor.getColumnIndex(TabelaVoluntarios.CAMPO_IDADE)
            ...
            val idade = cursor.getLong(volIdade)
            val telefone = cursor.getLong(volTelefone)
            val genero = cursor.getString(volGenero)

            return Voluntario(id, nome, idade, telefone, genero)
        }
    }
}